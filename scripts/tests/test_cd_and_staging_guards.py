...
def test_workflow_deploys_lambda_production_only_from_main_with_oidc() -> None:
    """Production Lambda deploys must be impossible on PRs or non-main branches."""
    findings = _findings()
    ...
    assert (
        "production Lambda promotion must provide Lambda deploy inputs" not in findings
    )
    assert (
...
def test_deploy_script_fails_fast_when_deploy_inputs_are_missing() -> None:
    ...
    result = subprocess.run(
        ...
        env={
            ...
            if k not in {"CI", "GITHUB_ACTIONS", "TF_VAR_vpc_id"}
        },
        text=True,
        capture_output=True,
        check=False,
