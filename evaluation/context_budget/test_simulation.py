...
from backend.app.context_budget import (
    SelectedContext,
    estimate_text_request_tokens,
    format_turn,
    select_history,
)
...
def test_replay_pins_twenty_spanish_exchanges_and_metadata() -> None:
    ...
    assert all(
        spanish_marks.intersection(exchange.answer.lower())
...
def test_replay_preserves_sources_and_exact_history_boundary() -> None:
    replay = load_replay(REPLAY_PATH)
    counter = PinnedCorpusTokenCounter(replay.token_counts)
    turn = replay.exchanges[0]
    exact_cost = counter.count(format_turn(turn, 1)) + 8
    known_overhead = estimate_text_request_tokens(
        counter,
        replay.metadata.system_prompt,
        replay.metadata.tools,
        turn.question,
    )

    def select(history_budget: int) -> SelectedContext:
        return select_history(
            turns=[turn],
            model=replay.metadata.model,
            config=ContextBudgetConfig(1.0, history_budget + known_overhead, 0, 0, 20),
            system_prompt=replay.metadata.system_prompt,
            tools=replay.metadata.tools,
            current_message=turn.question,
            token_counter=counter,
        )

    assert select(exact_cost).turns == (turn,)
    assert select(exact_cost - 1).turns == ()
    assert turn.source_documents == ("raw/paneles/jinko-tiger-neo.pdf",)
...
from evaluation.context_budget.simulation import generate_report, load_replay, serialize_report
sys.stdout.buffer.write(serialize_report(generate_report(load_replay())))
"""
    outputs = []
    for run in range(2):
