...
def check_hallucination(
    ...
    """
    # Criterion 1: Only check for hallucinations when sources were expected (num_sources == 0)
    # Skip this check for regular conversation which doesn't require technical documents
    if num_sources == 0:
        # Extract numerical values - if there are technical numbers with zero sources, it's hallucination
        ...
        # No technical values - not a hallucination, just regular conversation
        return False, "", []
...
def main() -> None:
    ...
    parser.add_argument(
        "--upload-s3", action="store_true", help="Upload results to S3 bucket"
    )
